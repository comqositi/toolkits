...
// NewNodeByEnvSno 使用系统环境变量创建node节点
func NewNodeByEnvSno() (*Node, error) {
	noStr := os.Getenv(envNodeKey)
	if noStr == "" {
		return nil, errors.New("snowflake env S_NO 未设置，无法初始化snowflake node")
	}
	no, err := strconv.Atoi(noStr)
	if err != nil {
		return nil, errors.New("snowflake S_NO 不是一个数字")
	}
	return NewNode(int64(no))
}
